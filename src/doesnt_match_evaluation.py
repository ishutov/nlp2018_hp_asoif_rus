...
doesnt_match_data = open(DOESNT_MATCH_FILE, encoding='utf-8').readlines()
...
def analyze_with_pandas(method, task_results):
    ...
    if DO_FREQ_EVAL:
        df.columns = ['task_type', 'task_terms', 'found_outlier', 'correct_outlier', 'found_tf', 'correct_tf', 'avg_tf',
                      'difficulty', 'correct']
    else:
